class SchedulePlanController < ApplicationController
  ...
  # curl -X POST -d "{\"doctor_id\":23,\"days\":[{\"weekday\":2, \"start\":600, \"end\":720}, {\"weekday\":6, \"start\":840, \"end\":1140}], \"start_date\":\"08-10-2011\"}" localhost:3000/schedule_plan/create
  ...
  def create
    ...
    @json["days"].each do |d|
      @sch.workdays.build :weekday => d["weekday"], #workdays << Workday.new( :weekday => d["weekday"],
                                   :start => d["start"],
                                   :end => d["end"] 
    end
